...
def fill_list_to_length(my_array, K):
    M = len(my_array)

    if M >= K:
        return my_array

    while len(my_array) < K:
        random_element = np.random.choice(my_array)
        my_array = np.append(my_array, random_element)
...
def get_backbone(p):
    ...
    if p['backbone'] == 'swim_transformer2':
        ...
        ### swin-S
        if p['model'] == 'MTFormer_nyud2_minigpt_fusio': # removed N at end on purpose
            backbone = SwinTransformer(pretrain_img_size=224, window_size=7, depths=(2, 2, 18, 2), num_heads=(3, 6, 12, 24),
                                   in_chans=13, embed_dim=96, drop_path_rate=0.3)
        else:
            backbone = SwinTransformer(pretrain_img_size=224, window_size=7, depths=(2, 2, 18, 2), num_heads=(3, 6, 12, 24),
                                   in_chans=3, embed_dim=96, drop_path_rate=0.3)
        pretrain_path = './pre_train/swin_small_patch4_window7_224.pth'
...
def get_model(p,):
    ...
    if p['setup'] == 'multi_task':
        if p['model']  == 'SingleTaskModelSemSeg':
            from models.models_nyud_CL import SingleTaskModelSemSeg
            output_list = {}
            for task in p.TASKS.NAMES:
                output_list[task] = p.TASKS.NUM_OUTPUT[task]
            model = SingleTaskModelSemSeg(backbone, output_list, p.TASKS.NAMES, backbone_channels)
        elif p['model']  == 'SingleTaskModelSemSeg_PASCAL':
            from models.models_pascal_CL import SingleTaskModelSemSeg_PASCAL
            output_list = {}
            for task in p.TASKS.NAMES:
                output_list[task] = p.TASKS.NUM_OUTPUT[task]
            model = SingleTaskModelSemSeg_PASCAL(backbone, output_list, p.TASKS.NAMES, backbone_channels)
        elif p['model']  == 'SingleTaskModelHuman_PASCAL':
            from models.models_pascal_CL import SingleTaskModelHuman_PASCAL
            output_list = {}
            for task in p.TASKS.NAMES:
                output_list[task] = p.TASKS.NUM_OUTPUT[task]
            model = SingleTaskModelHuman_PASCAL(backbone, output_list, p.TASKS.NAMES, backbone_channels)
        elif p['model']  == 'SingleTaskModelSaliency_PASCAL':
            from models.models_pascal_CL import SingleTaskModelSaliency_PASCAL
            output_list = {}
            for task in p.TASKS.NAMES:
                output_list[task] = p.TASKS.NUM_OUTPUT[task]
            model = SingleTaskModelSaliency_PASCAL(backbone, output_list, p.TASKS.NAMES, backbone_channels)
        elif p['model']  == 'MultiTaskModel_PASCAL':
            from models.models_pascal_CL import MultiTaskModel_PASCAL
            output_list = {}
            for task in p.TASKS.NAMES:
                output_list[task] = p.TASKS.NUM_OUTPUT[task]
            model = MultiTaskModel_PASCAL(backbone, output_list, p.TASKS.NAMES, backbone_channels)
        elif p['model']  == 'SingleTaskModelDepth_NYUD':
            from models.models_nyud_CL import SingleTaskModelDepth
            output_list = {}
            for task in p.TASKS.NAMES:
                output_list[task] = p.TASKS.NUM_OUTPUT[task]
            model = SingleTaskModelDepth(backbone, output_list, p.TASKS.NAMES, backbone_channels)
        elif p['model']  == 'SingleTaskModelNormal_NYUD':
            from models.models_nyud_CL import SingleTaskModelNormal
            output_list = {}
            for task in p.TASKS.NAMES:
                output_list[task] = p.TASKS.NUM_OUTPUT[task]
            model = SingleTaskModelNormal(backbone, output_list, p.TASKS.NAMES, backbone_channels)    
        elif p['model'] == 'MultiTaskModel_NYUD':
            from models.models_nyud_CL import MultiTaskModel_NYUD
            output_list = {}
            for task in p.TASKS.NAMES:
                output_list[task] = p.TASKS.NUM_OUTPUT[task]
            model = MultiTaskModel_NYUD(backbone, output_list, p.TASKS.NAMES, backbone_channels)
        else:
...
def get_train_dataset(p, transforms):
    ...
    if db_name == 'PASCALContext2':
        ...
        database = NYUD_MT(split='train', transform=transforms,
                                    ...
                                    do_semseg='semseg' in p.ALL_TASKS.NAMES,
                                    do_depth='depth' in p.ALL_TASKS.NAMES, do_reconstruction='reconstruction' in p.ALL_TASKS.NAMES, overfit=p['overfit'])
...
def get_train_dataloader(p, dataset):
    ...
    trainloader = DataLoader(dataset, batch_size=p['trBatch'], shuffle=False, sampler=DistributedSampler(dataset), drop_last=True,
...
def get_val_dataset(p, transforms):
    ...
    if db_name == 'PASCALContext':
        ...
        database = NYUD_MT(split='val', transform=transforms,
                                ...
                                do_depth='depth' in p.TASKS.NAMES, do_reconstruction='reconstruction' in p.ALL_TASKS.NAMES, overfit=p['overfit'])
    else:
...
def get_val_dataloader(p, dataset):
    ...
    testloader = DataLoader(dataset, batch_size=p['valBatch'], shuffle=False, sampler=DistributedSampler(dataset), drop_last=True,
...
def get_loss(p, task=None):
    ...
    if task == 'semseg' or task == 'human_parts':
        ...
        from losses.loss_functions import ReconstructionLoss
...
def get_criterion(p):
    ...
    if p['setup'] == 'multi_task':
        ...
        elif p['loss_kwargs']['loss_scheme'] == 'static': 
...
def adjust_learning_rate(p, optimizer, epoch):
    ...
    if p['scheduler'] == 'step':
        ...
        lr = CosineAnnealingLR(optimizer, T_max=1000, eta_min=0, last_epoch=epoch-1).get_last_lr()[0]
