...
class NYUD_MT(data.Dataset):
    def __init__(self,
        ...
        # Text Descriptions
        self.descriptions = []
        _descriptions_dir = os.path.join(root, 'descriptions')

        self.depth_descriptions = []
        _depth_descriptions_dir = os.path.join(root, 'depth_descriptions')
        ...
        _reconstruction_gt_dir = os.path.join(root, 'reconstruction')
        ...
        for splt in self.split:
            ...
            for ii, line in enumerate(lines):
                ...
                # Text Description
                #_text = os.path.join(self.root, _descriptions_dir, line + '.npy')
                #assert os.path.isfile(_text)
                #self.descriptions.append(_text)

                # Depth Text Description
                #_depth_description = os.path.join(self.root, _depth_descriptions_dir, line + '.npy')
                #assert os.path.isfile(_depth_description)
                #self.depth_descriptions.append(_depth_description)

                # Reconstruction Prediction
                #_reconstruction = _image # If supervised, it is same as the image folder
                #assert os.path.isfile(_reconstruction)
                #self.reconstructions.append(_reconstruction)
    ...
    def __getitem__(self, index):
        ...
        #_text = self._load_description(index)
        #_depth_description = self._load_description(index)
        sample['image'] = _img
        ...
        if self.do_reconstruction:
            _reconstruction = self._load_reconstruction(index)
            sample['reconstruction'] = _reconstruction
        ...
        sample['index'] = index
        #sample['description'] = _text
        #sample['depth_description'] = _depth_description
        return sample
    ...
    def _load_description(self, index):
        _text = np.load(self.descriptions[index]).astype(np.float32)
        return _text    

    def _load_depth_description(self, index):
        _text = np.load(self.depth_descriptions[index]).astype(np.float32)
        return _text 
    ...
    def _load_reconstruction(self, index):
        _reconstruction = np.array(Image.open(self.images[index]).convert('RGB')).astype(np.float32)
