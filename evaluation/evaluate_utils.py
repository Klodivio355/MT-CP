...
def calculate_multi_task_performance(eval_dict, single_task_dict):
    ...
    for task in tasks:
        ...
        if task == 'depth': # rmse lower is better
            ...
            mtl_performance -= (mtl['fid'] - stl['fid'])/stl['fid'] # fid lower is better
...
def get_single_task_meter(p, database, task):
    ...
    if task == 'semseg':
        ...
        from evaluation.eval_depth import EdgeMeter
        return EdgeMeter()

    elif task == 'reconstruction':
        from evaluation.eval_reconstruction import ReconstructionMeter
        return ReconstructionMeter()
...
def validate_results(p, current, reference):
    ...
    if len(tasks) == 1: # Single-task performance
        ...
        elif task == 'reconstruction': # Saliency estimation (mIoU)
            if current['reconstruction']['fid'] > reference['reconstruction']['fid']:
                print('New best reconstruction estimation model %.2f -> %.2f' %(100*reference['reconstruction']['fid'], 100*current['reconstruction']['fid']))
                improvement = True
            else:
                print('No new best reconstruction estimation model %.2f -> %.2f' %(100*reference['reconstruction']['fid'], 100*current['reconstruction']['fid']))
                improvement = False
        ...
        elif task == 'normals': # Depth estimation (rmse)
            if current['normals']['mean'] < reference['normals']['mean']:
                print('New best normals estimation model %.3f -> %.3f' %(reference['normals']['rmse'], current['normals']['rmse']))
                improvement = True
            else:
                print('No new best normals estimation model %.3f -> %.3f' %(reference['normals']['rmse'], current['normals']['rmse']))
                improvement = False
...
def save_model_predictions2(p, val_loader, model):
    ...
    for ii, sample in enumerate(val_loader):
        inputs, meta = sample['image'].cuda(non_blocking=True), sample['meta']
        img_size = (inputs.size(2), inputs.size(3))
        #texts = sample['description'].cuda(non_blocking=True)
        #depth_texts = sample['depth_description'].cuda(non_blocking=True)
        ...
        for task in p.TASKS.NAMES:
            ...
            for jj in range(int(inputs.size()[0])):
                ...
                if task in ['depth', 'edge']:
                    sio.savemat(os.path.join(save_dirs[task], fname + '.mat'), {'depth': result})
                else:
                    imageio.imwrite(os.path.join(save_dirs[task], fname + '.png'), result.astype(np.uint8))

@torch.no_grad()
def save_model_predictions3(p, val_loader, model):
    """ Save model predictions for all tasks """
...
def eval_all_results(p):
    ...
    if 'edge' in p.TASKS.NAMES:
        ...
        results['edge'] = eval_edge_predictions(database=p['val_db_name'],
                           save_dir=save_dir, overfit=p.overfit)
    ...
    if 'reconstruction' in p.TASKS.NAMES:
        from evaluation.eval_reconstruction import eval_reconstruction_predictions
        results['reconstruction'] = eval_reconstruction_predictions(database=p['val_db_name'],
                             save_dir=save_dir, overfit=p.overfit)
